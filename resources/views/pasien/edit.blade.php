...
<head>
    ...
    <script>
        var dob_day = "{{date('j',strtotime($pasien->tanggal_lahir))}}";
        var dob_month = "{{date('n',strtotime($pasien->tanggal_lahir))}}";
        var dob_year = "{{date('Y',strtotime($pasien->tanggal_lahir))}}";
    </script>
    <script src="{{asset('js/dobpicker.js') }}"></script>
    <script src="{{asset('js/editpasien.js') }}"></script>
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <nav class="container py-3">
            <ul class="flex text-bold">
                ...
                <li class="mr-3">
                    <a class="text-teal-500 hover:text-teal-800"
                        href="{{action('PasienController@show', $pasien->id)}}">{{ $pasien->nomor_rm }}</a>
                </li>
                <li class="mr-3">
                    <p class="text-teal-500">></p>
                </li>
                <li class="mr-3">
                    <p class="text-teal-500">Edit</p>
                </li>
            </ul>
        </nav>

        <div class="container mx-auto py-8 px-4">
            <p class="text-xl text-teal-500 font-bold">Edit Data Pasien</p>
            ...
            <form class="w-full max-w-lg my-8" method="post" action="{{action('PasienController@update', $id)}}">
                @method('PATCH')
                @csrf
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                        ...
                        <input
                            ...
                            id="firstname" name="first_name" type="text" value="{{$pasien->first_name}}">
                    </div>
                    <div class="w-full md:w-1/2 px-3">
                        ...
                        <input
                            class="border border-gray-400 appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                            id="lastname" name="last_name" type="text" value="{{$pasien->last_name}}">
                    </div>
                </div>
                ...
                <div class="flex flex-wrap -mx-3 mb-6">
                    <div class="w-full px-3">
                        ...
                        <div class="relative">
                            <select
                                class="border border-gray-400 block appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                id="jenis_kelamin" name="jenis_kelamin" value="{{$pasien->jenis_kelamin}}">
                                <option value="Laki-laki" @if($pasien->jenis_kelamin==='Laki-laki') selected='selected'
                                    @endif>Laki-laki</option>
                                <option value="Perempuan" @if($pasien->jenis_kelamin==='Perempuan') selected='selected'
