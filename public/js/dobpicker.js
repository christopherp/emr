jQuery.extend({
	...
	dobPicker: function (params) {
		...
		// set days
		if (params.dayDefault === "Hari") {
			dayElement.append("<option value=\"\">" + params.dayDefault + "</option>");
		}		
		for (var i = 1; i <= 31; i++) {
			var day = "" + i;
			var value = i > 9 ? "" + i : "0" + i;
			if(i==params.dayDefault){
				dayElement.append("<option selected='selected' value=\"" + value + "\">" + day + "</option>");
			}else{
		...
		if (params.monthDefault === "Bulan") {
			monthElement.append("<option value=\"\">" + params.monthDefault + " </option>");
		}			
		for (var i = 1; i <= 12; i++) {
			var month = months[i - 1];
			var value = i > 9 ? "" + i : "0" + i;
			if(i==params.monthDefault){
				monthElement.append("<option selected='selected' value=\"" + value + "\">" + month + "</option>");
			}else{
		...
		for (i = minimum; i >= maximum; i--) {
			...
			if(i==params.yearDefault){				
				yearElement.append("<option selected='selected' value=\"" + value + "\">" + year + "</option>");
			}else{
